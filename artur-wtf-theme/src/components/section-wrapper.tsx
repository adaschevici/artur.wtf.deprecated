...
const slant = {
  right: { clipPath: `polygon(0 15%, 100% 25%, 100% 85%, 0 75%)` },
  left: { clipPath: `polygon(0 16%, 100% 4%, 100% 82%, 0 94%)` }
}
...
const SectionWrapper = ({
  ...
  slantDirection,
  factor = 2
}: {offset: number; factor?: number, children?: React.ReactNode, slantDirection: string }) => (
  <div>
    <Divider
      bg="linear-gradient(to right, SlateBlue 0%, DeepSkyBlue 100%)"
      sx={slant[slantDirection]}
      speed={-0.2}
      offset={offset + 0.1}
      factor={factor}
    />
